...
df_pref_map['一人当たり賃金（相対値）'] = ((df_pref_map['一人当たり賃金（万円）']-df_pref_map['一人当たり賃金（万円）'].min())/(df_pref_map['一人当たり賃金（万円）'].max()-df_pref_map['一人当たり賃金（万円）'].min())) #正規化
...
st.header('■年齢階級別の全国一人あたり平均賃金（万円）')  #df_jp_ind
...
st.plotly_chart(fig) #streamlitからplotlyを呼び出す

#ダッシュボード最後　産業別平均賃金を横棒グラフで表示する
st.header('産業別の賃金推移')
...
max_x = df_mean_categ[option_wage].max() + 50 #グラフ最大値を値最大値＋50にする
